...
```powershell
aspire agent init --non-interactive --skills aspire,aspire-init,aspire-orchestration,aspire-monitoring,aspire-deployment,aspireify --skill-locations standard
```
...
- SQL Server: wait for the database; import the optional BACPAC only when the managed database is empty; build and publish the DACPAC; verify completion; then start the API.
- PostgreSQL: wait for the database; run the DbPublisher project with `Paradigm_ORM_ConnectionString` supplied from the Aspire resource; verify the schema before releasing dependents.
...
aspire logs <database-bootstrap>
```
