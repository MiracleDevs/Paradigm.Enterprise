...
- Publish every system catalog through a rerunnable post-deployment data script. Mirror its identifiers and stable machine codes in a service-side .NET enum; when the solution has an Interfaces project, prefer that cross-application owner over Domain. Give each member an explicit numeric value and stable serialized code, then test complete bidirectional seed parity so neither the database nor enum may contain an unreviewed extra member. Review database seed and enum changes together; a localized/display name may differ. Never delete, reuse, or renumber a published status identifier. Retire it with `IsActive` while preserving foreign-key targets.
