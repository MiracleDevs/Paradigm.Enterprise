...
| Invariants/mapping | existing and new partials outside `Entities` and `Context` | Move namespaces/usings as necessary, but retain behavior in `Domain/Sales`, `Domain/MasterData`, `Domain/Access`, and `Domain/Operations`, never in EFPT output. |
...
5. Amend `EntityType.t4` while retaining all current base-type/auditing, nullable, navigation, and filtered-index fixes. Detect EF view mappings from EF metadata. For a table `Product`, generate `: EntityBase<int>, IProduct`; for its keyless `ProductView`, generate `: IProduct`. Do the same for all nine DTO pairs. Internal helper views remain keyless types without a paired interface. Add the needed generated-interface namespace and fail template generation clearly if a `{Entity}View` lacks its base entity or lacks `Id`.
6. Amend `DbContext.t4` only as required to keep its `DbContextBase<int>` constructor/base call and correctly generate all table/view `DbSet`s and mapping. Preserve `OnModelCreatingPartial` and the CustomerAddress one-to-many correction. Do not move mapping into repositories or handwritten context code.
...
4. **Pricing helpers:** retain them as internal keyless types until Task 3 removes their repository consumers; do not make artificial `IQuotePricing` or `ISalesOrderPricing` contracts.
