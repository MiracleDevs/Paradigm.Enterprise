...
- `QuotePricing` and `SalesOrderPricing` remain plain keyless read shapes. Status catalogs, history, audit, and idempotency types do not receive artificial public view contracts.
- Collection `DomainTracker<TEntity>` members are emitted only for the owned `Quote.QuoteLines` and `SalesOrder.SalesOrderLines` aggregate collections. Other database relationships retain ordinary EF collections; a foreign key alone does not make an inverse navigation part of an aggregate. `AuditLog` and `IdempotencyRequest` also cannot satisfy the integer-entity tracker constraint because their persistence identities are long/composite.
