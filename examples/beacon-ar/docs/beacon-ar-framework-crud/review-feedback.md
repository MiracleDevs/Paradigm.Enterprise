...
5. **P2 SQL project identity/scope - resolved.** `ProjectGuid` and `TargetDatabaseSet` are absent, the nested `Folder` metadata remains without duplicating the `Build`/`None` globs, the reason is documented, the solution identity is unchanged, and the file ends with a newline. The Release solution build and strict database validator both pass.
