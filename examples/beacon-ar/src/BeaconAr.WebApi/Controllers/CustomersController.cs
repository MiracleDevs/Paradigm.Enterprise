...
public sealed class CustomersController : ControllerBase
{
    ...
    public async Task<ActionResult<PageResult<CustomerDto>>> Search(
    ...
    public async Task<ActionResult<CustomerDto>> Get(int id, CancellationToken cancellationToken)
    {
        ...
        CustomerDto value = await _provider.GetByIdAsync(id, cancellationToken);
    ...
    public async Task<ActionResult<CustomerDto>> Create([FromBody] CustomerCreateRequest request, [FromHeader(Name = "Idempotency-Key")] string? key, CancellationToken cancellationToken)
    {
        (CustomerDto value, bool replayed) = await _idempotency.ExecuteAsync("create-customer", key, request,
    ...
    {
        ApiContract.EnsurePositiveId(id);
        CustomerDto value = await _provider.UpdateAsync(id, request, ETagCodec.ParseRequired(ifMatch), cancellationToken);
        ApiContract.SetETag(Response, value.Version);
        return Ok(value);
    }
