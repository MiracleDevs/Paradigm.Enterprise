...
namespace Paradigm.Enterprise.Data.Context
{
    ...
    /// <remarks>
    /// When <see cref="SaveChangesAsync(CancellationToken)"/> is used, the audit hook is invoked for
    /// added and modified <see cref="IAuditableEntity{TId}"/> entries when an authenticated user is
    /// available from the current service scope. The default hook populates fields only for entities
    /// that also implement <see cref="IAuditableEntity{TDate,TId}"/> with <see cref="DateTime"/> or
    /// <see cref="DateTimeOffset"/> timestamps.
    /// Changes are persisted only when a save or <see cref="CommitChangesAsync"/> is called.
    /// The inherited synchronous <c>SaveChanges()</c> and <c>SaveChangesAsync(bool, CancellationToken)</c>
    /// overloads are not overridden here and therefore bypass this audit path.
        ...
        where TId : struct, IEquatable<TId>
    {
        ...
        /// <remarks>
        /// The context enumerates every tracked auditable entity and performs the authenticated-user
        /// lookup before checking the entry state. Only added and modified entries are mutated. If a
        /// lookup returns <see langword="null"/>, that entry is not audit-mutated and the lookup may be
        /// repeated for later auditable entries.
        ...
        /// <see cref="IAuditableEntity{TId}"/>. That extension populates standard fields only when
        /// <paramref name="entity"/> also implements the <see cref="DateTime"/> or
        /// <see cref="DateTimeOffset"/> timestamp audit contract; other timestamp contracts are unchanged.
