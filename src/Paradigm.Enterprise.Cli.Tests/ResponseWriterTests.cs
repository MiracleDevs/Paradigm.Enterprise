...
public class ResponseWriterTests
{
    ...
    public async Task Text_and_json_output_are_deterministic_and_json_has_stable_envelope()
    {
        var response = new CommandResponse(
            ...
            "api search",
            "success",
            [new("Paradigm.Enterprise.Data", "1.1.0", "Sample.csproj")],
            [new("member", "Sample.Z", "protected virtual void Z()", "Paradigm.Enterprise.Data", "1.1.0")],
            []);
    ...
    {
        var results = Enumerable.Range(0, 60)
            .Select(index => new ResultItem(
                "direct-package", $"Package.{index:D2}", "1.0.0", Project: $"Project.{index}.csproj"))
            .Append(new("direct-package", "Package.00", "1.0.0", Project: "Duplicate.csproj"))
            .ToArray();
        var response = new CommandResponse(
            "1.1", "packages check", "success", [], results, []);
