...
public class PackageAuditPolicyTests
{
    ...
    public void Vulnerable_deprecated_and_outdated_policies_are_classified()
    {
        ...
        var vulnerable = DotNetPackageAuditor.ReadFindings(
            json, "--vulnerable", "App.csproj", includePrerelease: false);
        var deprecated = DotNetPackageAuditor.ReadFindings(
            json, "--deprecated", "App.csproj", includePrerelease: false);
        var outdated = DotNetPackageAuditor.ReadFindings(
            json, "--outdated", "App.csproj", includePrerelease: false);

        CollectionAssert.AreEquivalent(
            new[] { "Direct.Stable", "Transitive.Stable" },
            vulnerable.Select(Name).ToArray());
        Assert.IsTrue(vulnerable.All(x => x.Code == "PE7006" && x.Severity == "error"));
        CollectionAssert.AreEquivalent(
            new[] { "Direct.Stable", "Transitive.Stable" },
            deprecated.Select(Name).ToArray());
    ...
    public void Prerelease_findings_are_included_only_when_configured()
    {
        ...
        Assert.IsEmpty(DotNetPackageAuditor.ReadFindings(
            json, "--outdated", "App.csproj", includePrerelease: false));
        Assert.HasCount(1, DotNetPackageAuditor.ReadFindings(
            json, "--outdated", "App.csproj", includePrerelease: true));
    }
    ...
    public async Task Private_feed_failure_is_an_incomplete_audit_error()
    {
        ...
        var configuration = new ParadigmConfiguration(
            "config.json", fixture.Root, [], new(), []);

        var diagnostics = await new DotNetPackageAuditor(fixture.Executable).AuditAsync(
            new([project], project), configuration, CancellationToken.None);

        Assert.HasCount(3, diagnostics);
        Assert.IsTrue(diagnostics.All(x =>
            x.Code == "PE7005" && x.Severity == "error" &&
            x.Message.Contains("private feed unavailable", StringComparison.Ordinal)));
    }
    ...
    private sealed class AuditFixture : IDisposable
    {
        ...
        public static AuditFixture Create()
        {
            ...
            var executable = Path.Combine(root,
                OperatingSystem.IsWindows() ? "CheckPackFixture.exe" : "CheckPackFixture");
