...
public class ArtifactResolutionTests
{
    ...
    public async Task Custom_target_path_staleness_and_malformed_output_are_not_false_clean()
    {
        ...
        try
        {
            ...
            Assert.IsNotNull(
                built.ApplicationAssembly,
                $"Expected evaluated output: {built.ExpectedApplicationAssembly}; actual DLLs: " +
                string.Join(", ", Directory.EnumerateFiles(root, "*.dll", SearchOption.AllDirectories)));
            ...
            Assert.IsTrue(json.RootElement.GetProperty("diagnostics").EnumerateArray()
