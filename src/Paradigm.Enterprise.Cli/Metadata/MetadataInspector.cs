...
internal sealed class MetadataInspector : IDisposable
{
    ...
    public MetadataInspector(AssetSelection selection)
    {
        ...
        var candidates = selection.MetadataPaths.Where(path => selection.InspectionAssemblyNames.Contains(Path.GetFileNameWithoutExtension(path))).Order(StringComparer.OrdinalIgnoreCase);
    ...
    private IReadOnlyList<string> GetVisibleMembers(Type type, string assemblyName)
    {
        ...
        foreach (var property in type.GetProperties(flags))
        {
            var accessor = new[]
            {
                property.GetMethod,
                property.SetMethod
            }.Where(x => x is not null).Cast<MethodInfo>().FirstOrDefault(IsVisible);
            if (accessor is null)
                continue;
            members.Add($"{Modifiers(accessor)} {TypeName(property.PropertyType)} {property.Name} {{ " + $"{Accessor(property.GetMethod, "get")} {Accessor(property.SetMethod, "set")}}}");
        }
    ...
    private IReadOnlyList<ApiMember> GetStructuredMembers(Type type, string assemblyName)
    {
        ...
        foreach (var property in type.GetProperties(flags))
        {
            ...
            var signature = $"{Modifiers(representative)} {TypeName(property.PropertyType)} {property.Name} {{ " + $"{Accessor(property.GetMethod, "get")} {Accessor(property.SetMethod, "set")}}}";
            var accessors = new[]
            {
                (Method: property.GetMethod, Kind: "get"),
                (Method: property.SetMethod, Kind: "set")
            }.Where(x => x.Method is not null).Select(x => new ApiAccessor(x.Kind, Visibility(x.Method!))).ToArray();
            members.Add(new("property", property.Name, signature, TypeName(property.PropertyType), accessors, Attributes(property), []));
        }

        foreach (var method in type.GetMethods(flags).Where(x => !x.IsSpecialName && IsVisible(x)))
            members.Add(new("method", method.Name, MethodSignature(method), TypeName(method.ReturnType), [], Attributes(method), Constraints(method.GetGenericArguments())));
        var docs = documentation.GetValueOrDefault(assemblyName);
        var typeDoc = docs?.GetValueOrDefault($"T:{type.FullName?.Replace('+', '.')}");
        if (!string.IsNullOrWhiteSpace(typeDoc))
    ...
    private static IReadOnlyList<InspectedAction> GetDeclaredActions(Type type)
    {
        ...
        return type.GetMethods(flags).Where(x => !x.IsSpecialName).Select(x => new InspectedAction(x.Name, MethodSignature(x), Attributes(x))).OrderBy(x => x.Signature, StringComparer.Ordinal).ToArray();
    }

    private static IReadOnlyList<string> Attributes(MemberInfo member) => CustomAttributeData.GetCustomAttributes(member).Select(attribute => TypeName(attribute.AttributeType)).Order(StringComparer.OrdinalIgnoreCase).ToArray();
    private static string MethodSignature(MethodInfo method)
    {
        var generic = method.IsGenericMethodDefinition ? $"<{string.Join(", ", method.GetGenericArguments().Select(x => x.Name))}>" : "";
    ...
    private static IReadOnlyList<ApiGenericConstraint> Constraints(IEnumerable<Type> arguments) => arguments.Where(x => x.IsGenericParameter).Select(argument =>
    {
        var constraints = argument.GetGenericParameterConstraints().Select(x => TypeName(x)).ToList();
        var attributes = argument.GenericParameterAttributes & GenericParameterAttributes.SpecialConstraintMask;
        if (attributes.HasFlag(GenericParameterAttributes.ReferenceTypeConstraint))
    ...
    {
        var modifier = parameter.ParameterType.IsByRef ? parameter.IsOut ? "out " : parameter.IsIn ? "in " : "ref " : "";
    ...
    private static string Visibility(MethodBase method) => method.IsPublic ? "public" : method.IsFamilyOrAssembly ? "protected internal" : method.IsFamily ? "protected" : method.IsAssembly ? "internal" : "private";
