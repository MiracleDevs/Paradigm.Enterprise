...
internal static partial class ProjectResolver
{
    public static ProjectSelection Resolve(string? input)
    {
        ...
        var solutions = Directory.EnumerateFiles(path, "*.sln", SearchOption.TopDirectoryOnly)
            .Concat(Directory.EnumerateFiles(path, "*.slnx", SearchOption.TopDirectoryOnly))
    ...
    private static IReadOnlyList<string> ReadSlnx(string path)
    {
        ...
        return XDocument.Load(path).Descendants("Project")
