...
#region Public Methods
    public static bool TryResolveWithin(string root, string relativePath, out string resolvedPath, out string? error)
    {
        try
        {
            ...
            if (OperatingSystem.IsWindows() && Path.GetExtension(candidate).Length == 0 && File.Exists(candidate + ".exe"))
                candidate += ".exe";
            resolvedPath = Canonicalize(candidate);
            var relative = Path.GetRelativePath(canonicalRoot, resolvedPath);
            var outside = Path.IsPathRooted(relative) || relative.Equals("..", StringComparison.Ordinal) || relative.StartsWith(".." + Path.DirectorySeparatorChar, StringComparison.Ordinal) || relative.StartsWith(".." + Path.AltDirectorySeparatorChar, StringComparison.Ordinal);
...
#region Private Methods
    internal static string Canonicalize(string path)
    {
        ...
        var root = Path.GetPathRoot(fullPath) ?? throw new ArgumentException($"Path '{path}' has no filesystem root.", nameof(path));
        var current = root;
