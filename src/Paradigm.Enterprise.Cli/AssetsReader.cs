...
internal static partial class AssetsReader
{
    public static AssetSelection Read(string project, string? requestedFramework)
    {
        ...
        using (document)
        {
            ...
            var packageFolders = root.TryGetProperty("packageFolders", out var folders)
                ? folders.EnumerateObject().Select(x => x.Name).Order(StringComparer.OrdinalIgnoreCase).ToArray()
                : [];
    ...
    internal static string SelectFramework(IEnumerable<string> targetValues, string? requested)
    {
        var targets = targetValues
            .Where(x => x.StartsWith("net", StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(FrameworkKey, StringComparer.OrdinalIgnoreCase)
        ...
        var exact = targets.FirstOrDefault(x =>
            ...
            x.StartsWith(requested + "/", StringComparison.OrdinalIgnoreCase));
        if (exact is not null)
            return exact;

        var requestedVersion = NetVersion(requested);
        var compatible = requestedVersion is null
            ? null
            : targets.FirstOrDefault(x => NetVersion(x) is { } candidate && candidate <= requestedVersion);
        return compatible ?? throw new AssetsException(
            $"Framework '{requested}' has no compatible restored target. Available targets: {string.Join(", ", targets)}.");
    }

    internal static IReadOnlyList<string> NormalizeMetadataPaths(
        IEnumerable<string> paths,
        string? applicationAssembly,
        IReadOnlyDictionary<string, (string Name, string Version)> owners,
        ...
        ICollection<Diagnostic> diagnostics)
    {
        ...
        return valid
            ...
            .Select(group => group.OrderBy(x => x.Rank).ThenBy(x => x.Path, StringComparer.OrdinalIgnoreCase).First().Path)
    ...
    private static IReadOnlyDictionary<string, string> ReadLibraryTypes(JsonElement root)
    {
        ...
        return libraries.EnumerateObject().ToDictionary(
            x => x.Name,
            x => x.Value.TryGetProperty("type", out var type) ? type.GetString() ?? "" : "",
            StringComparer.OrdinalIgnoreCase);
    }

    private static int PathPreference(
        string path,
        string? applicationAssembly,
        IReadOnlyDictionary<string, (string Name, string Version)> owners)
    {
    ...
    private static Version? NetVersion(string value)
    {
        ...
        if (!match.Success)
            return null;
        return Version.TryParse(match.Groups[1].Value.Contains('.') ? match.Groups[1].Value : match.Groups[1].Value + ".0", out var version)
            ? version
            : null;
    }
    ...
    private static (string? ExistingPath, string? ExpectedPath, bool IsStale) ResolveApplicationAssembly(string project, string framework)
    {
        ...
        foreach (var configuration in new[] { "Debug", "Release" })
        {
            ...
            if (!string.IsNullOrWhiteSpace(targetPath))
                candidates.Add(Path.IsPathRooted(targetPath)
                    ? Path.GetFullPath(targetPath)
                    : Path.GetFullPath(Path.Combine(Path.GetDirectoryName(project)!, targetPath)));
        }
    ...
    private static bool IsStale(string project, string assembly)
    {
        ...
        return Directory.EnumerateFiles(root, "*.cs", SearchOption.AllDirectories)
            ...
            .Any(path => File.GetLastWriteTimeUtc(path) > outputTime);
    }
    ...
    private static string ReadAssemblyName(string project)
    {
        try
        {
            return XDocument.Load(project).Descendants("AssemblyName").Select(x => x.Value).FirstOrDefault()
