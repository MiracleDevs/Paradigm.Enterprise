...
internal sealed class SemanticVersion : IComparable<SemanticVersion>, IEquatable<SemanticVersion>
{
    ...
    public static bool TryParse(string? value, out SemanticVersion version)
    {
        ...
        if (coreParts.Length != 3 ||
            !TryNumeric(coreParts[0], out var major) ||
            !TryNumeric(coreParts[1], out var minor) ||
            !TryNumeric(coreParts[2], out var patch) ||
            prereleaseText is not null && !ValidIdentifiers(prereleaseText, true))
            return false;
    ...
    private static bool TryNumeric(string value, out BigInteger number)
    {
        ...
        return value.Length > 0 &&
               ...
               BigInteger.TryParse(value, out number);
    }
