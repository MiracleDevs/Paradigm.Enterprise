...
Source-ownership policies that require resolved calls and type hierarchy, such as the `PE3107` repository SQL boundary, belong in `checks run`, not metadata-only `validate`. Run ownership checks over the post-generator compilation tree set while preserving handwritten-only exclusions for layout rules; a checked-in `.g.cs` file does not prove actual source-generator coverage. Analysis fails closed before rules run when the project compilation is incomplete; never fall back to method-name, filename, folder, parameter-name, or SQL-keyword-only guesses. Use resolved operations for conditional access and argument-to-parameter association. Once a connection extension's resolved receiver, method family, and string/`FormattableString` command parameter establish a raw-command sink, do not require the runtime argument to fold to recognizable SQL. Keep conservative SQL-shape recognition for repository-owned declarations/assignments without introducing broad taint analysis. Keep deliberately invalid semantic cases in focused fixture projects, cover production/test-project boundaries explicitly, and test allowed typed stored-procedure wrappers and ordinary business methods alongside prohibited sinks. Lock semantic policy fixtures with an exact ordered diagnostic manifest, including code, severity, member, construct, file, line, and column. An expired suppression must report `PE7004` without preventing the underlying semantic checks from running, while unrelated malformed configuration must still prevent built-in execution.
...
Before handing off a CLI change, format the affected projects, build and run the fast `Paradigm.Enterprise.Cli.Tests` set with `--filter "TestCategory!=Integration"`, run the installed tool's built-in checks against the CLI, checks, generator, and test projects, run the skill and plugin validators when guidance changed, and build DocFX with warnings as errors. Run `bash build/quality.sh` from any directory for the complete local equivalent. The script uses isolated temporary package and tool directories, packs the solution once, installs that CLI package, supplies `PARADIGM_CLI_INTEGRATION_PACKAGE` and `PARADIGM_CLI_INTEGRATION_EXECUTABLE` to the integration category, and then performs the example validation, deterministic checks, and documentation build used by the PR quality workflow.
