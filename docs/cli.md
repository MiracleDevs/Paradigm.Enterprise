...
| `PE3107` | A production `IRepository` implementation owns handwritten SQL, invokes an EF raw-SQL API, or constructs/executes a raw ADO or connection-extension command |
...
`PE3107` is an error from complete semantic compilation. It identifies repositories by assignability to `IRepository`, not by file, folder, namespace, or type name, and analyzes the post-generator compilation so source-generated production repositories are covered. EF/LINQ CRUD and bounded queries are allowed, as are calls to typed SQL Server or PostgreSQL stored-procedure wrappers whose SQL object lives in the database project. Production repository code may not use `FromSql*`, `ExecuteSql*`, `SqlQuery*`, raw ADO commands, Dapper-like SQL-bearing connection extensions, or repository-owned SQL statement strings—including SQL assigned to fields/properties after declaration. A resolved `Query*`/`Execute*` extension over `DbConnection`/`IDbConnection` with a string or `FormattableString` parameter is a prohibited raw-command sink even when the argument is a runtime value; parameter names and foldable SQL content are not required. Projects with `IsTestProject=true` are excluded; generated production repositories are not. There is no fixer because choosing EF or designing a typed database routine requires review.

An exceptional compatibility boundary uses the normal exact, expiring suppression contract. Prefer both the stable member and source location so the suppression cannot spread:

```json
{
  "code": "PE3107",
  "symbol": "Contoso.Sales.LegacyOrderRepository.SearchAsync",
  "location": "LegacyOrderRepository.cs(42,15)",
  "reason": "Remove after the typed SearchOrder routine is deployed.",
  "expires": "2026-12-31"
}
```

An expired suppression emits `PE7004` and no longer hides the underlying `PE3107`.
...
| `PE3101` to `PE3107` | Entity, repository, query, mutation, source-layout, or repository SQL ownership rule |
