...
Use `dotnet tool install --global Paradigm.Enterprise.Cli` only for evaluation. During repository development, pack to a local feed and pass `--add-source <artifacts>` to `dotnet tool install`; consumers should install the published package from the configured NuGet source.
...
Generated `.g.cs`, `.generated.cs`, `.designer.cs`, and `<auto-generated>` files are excluded from `PE3105`/`PE3106`. Run these checks against consuming applications. Framework internals and negative test fixtures intentionally contain patterns the application-facing rules reject.
