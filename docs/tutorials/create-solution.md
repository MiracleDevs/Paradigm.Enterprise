...
Review the inventory and rerun without `--dry-run`. Preserve the generated `.sln` or `.slnx` format. The CLI also creates `start.sh`, which restores/installs repository-local Paradigm and Aspire tools and checks Docker before starting Aspire; the database-neutral starter never requires SQLCMD or SqlPackage on the host. It creates a baseline `.github/workflows/quality.yml` and `.github/problem-matchers/paradigm.json`, so Paradigm warnings and errors appear as GitHub annotations. If you replace the workflow, preserve its `::add-matcher::` registration step before any Paradigm commands; see [GitHub Actions diagnostics](../cli.md#github-actions-diagnostics). Use `paradigm-build-database` to create the selected SQL Server or PostgreSQL project under `src/database` and add it to the application solution. Use `paradigm-setup-aspire` to add AppHost, ServiceDefaults, root `.env` handling, and database bootstrap ordering. For SQL Server, adapt its governed Dockerfile assets so the repository-built finite container owns the DACPAC build, SQLCMD 18, and pinned SqlPackage.
